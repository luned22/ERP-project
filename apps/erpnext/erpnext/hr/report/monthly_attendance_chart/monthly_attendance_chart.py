...
def execute(filters=None):
	...
	for emp in sorted(att_map):
		...
		department.setdefault(emp_det.department , []).append(round(average_time,1))
	...
	for x in department:
		total = 0
		for y in department[x]:
			total += y
		
		totalwh = total / len(department[x])
		print "department = %s , %s" % (x, department[x])
		print "total = %s" % total
		print "average = %s" % totalwh
		print 
